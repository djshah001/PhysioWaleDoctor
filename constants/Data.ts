import icons from "./icons";
import images from "./images";

export const OnBoardingItems = [
  {
    id: "1",
    title: "Mobility Drills/Test",
    description:
      "With our joint Specific self-assessment feature,you can easily evaluate your mobility right at home.",
    image: images.runner,
  },

  {
    id: "2",
    title: "Smart Point of Sale",
    description:
      "Complete point of sale software tailored to your business needs.",
    image: images.fitness,
  },
  {
    id: "3",
    title: "Instant Notifications",
    description:
      "Instant notifications let your quickly see new purchases and messages.",
    image: images.meditation,
  },
];

export const HorizontalList = [
  {
    id: "1",
    title: "Sponsored Physio",
    description:
      "Yorem ipsum dolor sit amet, consectetur adipiscing elit. Nunc vulputate libero et velit interdum, ac aliquet odio mattis.",
    image: images.doctor,
  },
  {
    id: "2",
    title: "Sponsored Physio",
    description:
      "Yorem ipsum dolor sit amet, consectetur adipiscing elit. Nunc vulputate libero et velit interdum, ac aliquet odio mattis.",
    image: images.doctor,
  },
  {
    id: "3",
    title: "Sponsored Physio",
    description:
      "Yorem ipsum dolor sit amet, consectetur adipiscing elit. Nunc vulputate libero et velit interdum, ac aliquet odio mattis.",
    image: images.doctor,
  },
];

export const IconMenuData = [
  { id: 1, title: "Self-Test", icon: icons.rehabGuide, route: "/self-test" },
  { id: 2, title: "Rehab-Guide", icon: icons.selfTest, route: "/rehab-guide" },
  { id: 3, title: "Yoga", icon: icons.yoga, route: "/yoga" },
  // { id: 4, title: "Booking", icon: icons.booking },
  {
    id: 5,
    title: "Appointments",
    icon: icons.booking,
    route: "/appointments/my-appointments",
  },
];

export const TestSteps = [
  { description: "Check Your Mobility With Our Simple 4 Steps" },
  { description: "Please click on the joint that is painful." },
  { description: "Perform the movement shown in the video." },
  {
    description: "Then fill out the questionnaire provided after the video.",
  },
  {
    description:
      "Based on your score, we will guide you to book an appointment with a physiotherapist.",
  },
];
