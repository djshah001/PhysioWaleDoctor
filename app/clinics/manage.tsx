...
import { useClinicDetails, useUpdateClinic } from "~/apis/hooks/useClinics";
...
const ClinicManagement = () => {
  ...
  const { data: clinicData, isLoading: loadingClinic } =
    useClinicDetails(editId);
  const { mutateAsync: updateClinic } = useUpdateClinic();

  const [currentStep, setCurrentStep] = useState(0);
  const [loading, setLoading] = useState(false);

  // Use editId directly, no need for pre-generated ID
  const clinicId = editId;
  ...
  React.useEffect(() => {
    ...
    if (data?.clinic) {
      ...
      setPhoneNumber(clinic.phoneNumber || user?.phoneNumber || "");
      setEmail(clinic.email || "");
      setWebsite(clinic.website || "");
      setConsultationFee(clinic.consultationFee?.toString() || "500");

      setAddress(clinic.address || "");
      setCity(clinic.city || "");
      setState(clinic.state || "");
      setCountry(clinic.country || "India");
      setPincode(clinic.pincode || "");
      if (clinic.location?.coordinates) {
        setCoordinates(clinic.location.coordinates as [number, number]);
      }

      setOpen24hrs(clinic.open24hrs || false);
      if (clinic.timing) {
        setTiming((prev) => ({ ...prev, ...clinic.timing }));
      }

      if (clinic.appointmentConfig) {
        setAppointmentConfig(clinic.appointmentConfig);
      }
      if (clinic.homeVisitConfig) {
        setHomeVisitConfig(clinic.homeVisitConfig);
      }

      setClinicType(clinic.clinicType || "Private Clinic");
      setSpecializations(clinic.specializations || []);
      setTags(clinic.tags || []);
      setFacilities(clinic.facilities || []);

      if (data.services) {
        setServices(data.services as any);
      }
      setPlatformCommissionRate(clinic.platformCommissionRate || 10);
      setGstNumber(clinic.gstNumber || "");
      if (clinic.socialLinks) {
        setSocialLinks(clinic.socialLinks);
      }

      // Pre-fill existing images as read-only or fully formed asset objects
      if (clinic.images && clinic.images.length > 0) {
        const existingImages = clinic.images.map((url: string) => ({
          asset: { uri: url } as ImagePicker.ImagePickerAsset,
          uploadedUrl: url,
          uploading: false,
          error: null,
        }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Prepare clinic data with all fields
      const clinicPayload = {
        name,
        description,
        address,
      ...
      await updateClinic({ id: editId, data: clinicPayload });
